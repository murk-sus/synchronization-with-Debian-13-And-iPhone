...
> Если Settings-панель глючит, **весь твик можно полностью настроить через терминал** (см. раздел «Команды (терминал, без Settings)»).
...
## Команды (терминал, без Settings)

```bash
# Проверить состояние демона и конфиг
/var/jb/usr/bin/debian-sync service status
/var/jb/usr/bin/debian-sync config show

# Настроить через терминал
su -c '/var/jb/usr/bin/debian-sync config set REMOTE_HOST 192.168.1.10'
su -c '/var/jb/usr/bin/debian-sync config set REMOTE_USER your_debian_user'
su -c '/var/jb/usr/bin/debian-sync config interval 60'

# Включить/выключить автосинк
su -c '/var/jb/usr/bin/debian-sync service enable'
su -c '/var/jb/usr/bin/debian-sync service disable'
